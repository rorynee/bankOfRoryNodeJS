...
router.get('/:id', function(req, res, next) {
    ...
    if(new String(req.session.user_id).valueOf() === new String(req.params.id).valueOf() || req.session.role === 1){
        ...
        pool.getConnection(function(err, connection) {
                    ...
                    res.render('deposit', { title: 'Dashboard - Deposit', sess: req.session, user: userdetails });  
...
router.put('/:id', function(req, res, next) {
	...
	if(validator.isFloat(amount,{ min: 0.01, max: 9999999999999.99 } )){
                        ...
                        pool.getConnection(function(err, connection) {
                                ...
                                // CALL GET_ACCOUNT_INFO(req.params)
                                var query = connection.query('CALL DEPOSIT_MONEY(?,?)',
                                ...
                                if(rows.affectedRows === 1){
                                           ...
                                           userdetails = rows[0][0];
                                           var success =  "Thank You. Your Money has been Deposited";
                                           res.render('deposit', { title: 'Dashboard - Deposit', sess: req.session, user: userdetails, success: success }); 
                                           //connection.release();
                                       });
                               }else{
